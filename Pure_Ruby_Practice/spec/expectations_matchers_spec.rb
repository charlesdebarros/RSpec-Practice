...
describe 'Expectation Matchers' do
  describe 'equivalence matchers' do
    it 'will match loose equality with #eq' do
      ...
      c = 17
      d = 17.0
      expect(c).to eq(d)    # different object types but 'close enough'
    end
    ...
    it 'will match value equality with #eql' do
      ...
      c = 17
      d = 17.0
      expect(c).not_to eql(d) # not the same type, 'close enough' does not count anymore
    end
    ...
    it 'will match identity equality with #equal' do
      ...
      c = b
      expect(b).to equal(c) # same object 'c' only 'points' to 'b'
  ...
  describe 'collection matchers' do
    it 'will match arrays' do
      ...
      expect(array).not_to contain_exactly(1, 2) # but with individual args
    end
    ...
    it 'will match hashes' do
      ...
      expect(hash).not_to include({ 'a' => 1, 'c' => 3 }) # 'a' and a: are not the same,
      # Rails would allow this comparison though.
    end
  end
  ...
  describe 'other useful matchers' do
    ...
    it 'will match class instances with #have_attributes' do
      ...
      expect(car).to have_attributes(colour: 'green')
      expect(car).to have_attributes(make: 'Ford', year: 2010, colour: 'green')
    end
  ...
  describe 'predicate matchers' do
    it 'will match be_* to custom methods ending in "?"' do
      # drops "be_", and adds "?" to end, calls method on object
    ...
    it 'will match have_* to custom methods like "has_*?"' do
      ...
      expect(customer).to have_pending_order          # customer.has_pendind_order?
  ...
  describe 'observation matchers' do
    ...
    # process to take pace inside of the expectation
    ...
    it 'will match when events change any value' do
      # calls the test before the block, then again
      # after the block
  ...
  describe 'composing matchers' do
    ...
    it 'will match by sending matchers as arguments to matchers' do
      ...
      expect(hash).to include(a: be.positive?, b: be_within(2).of(4))
    end
